<?php
/**
 * API para registrar un nuevo animal
...
try {
    ...
    if (empty($nombre) || empty($tipo) || empty($sexo) || empty($edad) || empty($tamano)) {
    ...
    $sql = "INSERT INTO animales (nombre, tipo, raza, sexo, edad, tamano, peso, color, descripcion, foto, estado, fecha_ingreso, observaciones, responsable_id) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    
    $stmt = $pdo->prepare($sql);
    $result = $stmt->execute([
        $nombre, $tipo, $raza, $sexo, $edad, $tamano, $peso, $color, 
        $descripcion, $foto, $estado, $fecha_ingreso, $observaciones, $user_id
    ]);
    ...
    if ($result) {
        ...
        throw new Exception('Error al registrar el animal');
    }
    
} catch (PDOException $e) {
    error_log("Error en registrar_animal.php: " . $e->getMessage());
    echo json_encode([
        'success' => false,
        'message' => 'Error en el servidor'
    ]);
} catch (Exception $e) {
    error_log("Error general en registrar_animal.php: " . $e->getMessage());
    echo json_encode([
        'success' => false,
